...
void Widget::on_m_DateSavePlushButton_clicked()
{
    ...
    QDateTime datetime = QDateTime::currentDateTime();
    QString str = QString("date -s \"%1-%2-%3 %4:%5:%6\"").arg( date.year()).arg(date.month()).arg(date.day()).arg(datetime.time().hour()).arg(datetime.time().minute()).arg(datetime.time().second());
...
void Widget::on_m_ExitPushButton_clicked()
{
    this->close();
}
...
{
    QString str = QString("date -s \"%1:%2:%3\"").arg(m_HourComboBox->currentIndex()).arg(m_MinuteComboBox->currentIndex()).arg(m_SecendsComboBox->currentIndex());
