...
WpaManager::WpaManager(QObject *parent):QObject(parent)
{
    ...
    wifiWid->setStatusText(tr("Connecting to wpa_supplicant service"));
...
void WpaManager::updateStatus()
{
    ...
    if (ctrl_conn == NULL || ctrlRequest("STATUS", buf, &len) < 0) {
        wifiWid->setStatusText(tr("Could not get status from ""wpa_supplicant"));
        ...
        if (pos) {
            *pos++ = '\0';
            if (strcmp(start, "bssid") == 0) {
                ...
                status_updated = true;
                wifiWid->setStatusText(wpaStateTranslate(pos));
    ...
    if (status_updated && mode)
        wifiWid->setStatusText(wifiWid->getStatusText() + " (" + mode + ")");
...
void WpaManager::updateNetworks()
{
    ...
    QListWidget *networkList = wifiWid->m_tabNetworkManager->m_networkList;
...
{
    QMessageBox::about(mainWindow, "wpa_gui for wpa_supplicant",
...
void WpaManager::processMsg(char *msg)
{
    ...
    //                tr("Disconnected from network."));
        ...
        //                       "authorized");
        if (wifiWid->getStatusText() == "INACTIVE" ||
                wifiWid->getStatusText() == "DISCONNECTED")
        {}
        //            wpaguiTab->setCurrentWidget(wpsTab);
...
void WpaManager::saveConfig()
{
    ...
    if (str_match(buf, "FAIL"))
        QMessageBox::warning(
                    mainWindow, tr("Failed to save configuration"),
        ...
        QMessageBox::information(
                    mainWindow, tr("Saved configuration"),
