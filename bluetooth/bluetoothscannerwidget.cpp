...
void BluetoothScannerWidget::itemActivated(QListWidgetItem *item)
{
    ...
    if (pairingStatus == QBluetoothLocalDevice::Paired || pairingStatus == QBluetoothLocalDevice::AuthorizedPaired )
    {
        int result = CMessageBox::showCMessageBox(mainWindow,"UnPair with "+btName.append("?"),"UnPair","Cancel");
    ...
    {
        int result = CMessageBox::showCMessageBox(mainWindow,"Pair with "+btName.append("?"),"Pair","Cancel");
        if(result == RESULT_CONFIRM)
        {
